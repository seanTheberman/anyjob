...
export function EmergencyJobsSection() {
  return (
    <section className="relative left-1/2 w-[min(calc(100vw-2rem),68rem)] -translate-x-1/2 overflow-hidden rounded-3xl bg-gray-950 text-white shadow-2xl">
      <div className="grid gap-0 lg:h-[26rem] lg:grid-cols-[45%_55%]">
        <div className="relative min-h-[20rem] p-6 sm:p-8 lg:min-h-0 lg:p-10">
          <Image
          ...
          <div className="relative z-10 flex min-h-[inherit] flex-col justify-between">
            <div>
              ...
              <h2 className="max-w-[360px] text-3xl font-extrabold leading-tight sm:text-4xl lg:text-5xl">
                Need someone urgently?
              </h2>
              <p className="mt-5 max-w-[390px] text-sm leading-6 text-gray-200 sm:text-base">
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            {EMERGENCY_STEPS.map(([number, title, detail]) => (
              <div key={number} className="rounded-2xl bg-white/10 p-3 ring-1 ring-white/10">
                <div className="mb-2 flex h-8 w-8 items-center justify-center rounded-full bg-red-500 text-sm font-extrabold text-white">
                  {number}
                </div>
                <h3 className="text-sm font-extrabold">{title}</h3>
                <p className="mt-1 text-xs leading-5 text-gray-300">{detail}</p>
              </div>
            ))}
          </div>
          ...
          <div className="mt-3 grid grid-cols-1 gap-3">
            {EMERGENCY_SERVICES.map((service) => (
              <a
                key={service.title}
                href={`tel:${ANYJOB_EMERGENCY_PHONE}`}
                className="group grid gap-3 overflow-hidden rounded-2xl bg-white/10 p-2.5 ring-1 ring-white/10 transition hover:bg-white/15 sm:grid-cols-[104px_1fr_auto] sm:items-center"
              >
                <div className="relative h-24 overflow-hidden rounded-xl sm:h-20">
                  <Image
                    src={service.image}
                    alt={service.title}
                    fill
